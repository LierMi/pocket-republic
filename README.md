...
- 可编辑个人宪法，A2-A5 会实际执行单笔限额、月度预算、强情绪拒付、学习里程碑声明和冷静期规则。
...
- 服务已经交付但 Receipt / Session usage 尚未刷新时，月度预算按已验证报价保守占用；公报仍显示 `Receipt 待确认`，不会把报价冒充实付金额。
- 国度月度预算属于 Pocket Republic 应用层政策，可由用户清除本地数据后重建；Kite 负责强制执行当前 Spending Session 的金额、时限、资产与 endpoint 边界。
- 探索成长国度的 MVP 只记录立宪者本人提交的里程碑完成声明；学院考官与外部证明验证明确属于 Roadmap v0.2。
...
- Receipt 延迟时的保守预算占用、沙盒/真实预算隔离和冷静期作用域
